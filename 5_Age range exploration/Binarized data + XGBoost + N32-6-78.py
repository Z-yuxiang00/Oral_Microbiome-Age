...
def age_groups(y, groups):
    bins = np.concatenate(([6], groups, [78]))
...
for i in range(6, N + 1):
    if i == 1:
        groups = list(range(N + 1, 78, N))
    else:
        groups = list(range(i, 79, N))
...
# 保存结果
test_result_df.to_excel("预测结果_age range_6-78.xlsx", index=False)