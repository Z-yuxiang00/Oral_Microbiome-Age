...
abundance = pd.read_csv('Genus_aggregated_data_relative.csv')
...
grouped_samples = (
    sample_order
    .join(pivot_data[['d__Bacteria;  p__Proteobacteria;  c__Gammaproteobacteria;  o__Burkholderiales;  f__Neisseriaceae;  g__Neisseria']])
    .sort_values(by=['Age_group', 'd__Bacteria;  p__Proteobacteria;  c__Gammaproteobacteria;  o__Burkholderiales;  f__Neisseriaceae;  g__Neisseria'])
)
...
plt.savefig('Genus.jpg', format='jpg', dpi=300)
plt.show()
merged_output = pd.concat([sample_order, pivot_data], axis=1)
merged_output.to_csv('sample_genus_abundance.csv', index=True)
