...
os.chdir(r"pathway")
data = pd.read_excel(
    r"Input_2550_samples+616ASV.xlsx",
    header=0
)
...
def age_groups(y, groups):
    bins = np.concatenate(([1], groups, [90]))
...
for N in N_values:
    for i in range(1, N + 1):
        if i == 1:
            groups = list(range(N + 1, 90, N))
        else:
            groups = list(range(i, 91, N))
...
for train_index, test_index in loo.split(X):
    ...
    for groups in classifiers:
        ...
        clf = RandomForestClassifier(n_estimators=200, random_state=8)
        clf.fit(X_train, y_train_groups)
        models.append((clf, groups))

    votes_test = np.zeros((90,), dtype=int)
...
results_combined.to_excel(
    r"predicted_vs_actual_RF_2550.xlsx",
    index=False
)
print("Finish")
